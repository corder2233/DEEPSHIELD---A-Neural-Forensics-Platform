...
# ─────────────────────────────────────────────
# ★ CUSTOM BACKGROUND IMAGE — paste your URL here
# ─────────────────────────────────────────────
BG_IMAGE_URL = "https://images.unsplash.com/photo-1677442135703-1787eea5ce01?w=1920&q=80"
# Replace the URL above with your own image link, e.g.:
# BG_IMAGE_URL = "https://your-server.com/your-image.jpg"
...
body::before {{
    ...
    background:
        radial-gradient(ellipse 80% 50% at 20% 20%, rgba(0,50,110,0.4) 0%, transparent 65%),
        radial-gradient(ellipse 60% 40% at 80% 80%, rgba(80,0,20,0.3) 0%, transparent 65%),
        #010204;
    pointer-events: none;
}}
...
.scanlines {{
    ...
    background: repeating-linear-gradient(
        0deg, rgba(0,0,0,0) 0px, rgba(0,0,0,0) 3px, rgba(0,0,0,0.03) 4px
    );
}}

/* ── HERO ── */
.hero-wrap {{
    text-align: center; padding: 3.5rem 1rem 1.8rem;
...
@keyframes hero-glow {{
    from {{ filter: drop-shadow(0 0 18px rgba(0,245,255,0.35)); }}
    to   {{ filter: drop-shadow(0 0 40px rgba(255,26,26,0.45)); }}
}}
.hero-sub {{
    font-family: 'Share Tech Mono', monospace;
    font-size: 0.78rem; letter-spacing: 4px;
    color: var(--text-muted); text-transform: uppercase; margin-top: 0.9rem;
    animation: fade-up 1.5s ease 0.2s both;
}}
.hero-line {{
    width: 200px; height: 1px;
    background: linear-gradient(90deg, transparent, var(--neon-cyan), var(--neon-red), transparent);
...
.neon-card:hover {{
    ...
    box-shadow: 0 0 22px rgba(0,245,255,0.09);
}}
.neon-card-red::before {{
    background: linear-gradient(90deg, transparent, var(--neon-red), transparent);
}}
.neon-card-red:hover {{
    border-color: rgba(255,26,26,0.45);
    box-shadow: 0 0 22px rgba(255,26,26,0.09);
}}
...
.sec-header {{
    ...
    letter-spacing: 2px; color: var(--neon-cyan); text-transform: uppercase;
...
/* ── BADGES ── */
.badge {{
    font-family: 'Share Tech Mono', monospace; font-size: 0.58rem; letter-spacing: 2.5px;
    background: rgba(0,245,255,0.08); border: 1px solid rgba(0,245,255,0.25);
    border-radius: 4px; padding: 0.25rem 0.65rem;
    color: var(--neon-cyan); text-transform: uppercase; display: inline-block; margin-bottom: 0.9rem;
}}
...
.result-real {{
    ...
    border: 1px solid rgba(57,255,20,0.42); border-radius: 16px; padding: 2.5rem; text-align: center;
    box-shadow: 0 0 45px rgba(57,255,20,0.14);
    animation: result-pop 0.4s ease both;
}}
.result-fake {{
    background: linear-gradient(135deg, rgba(28,0,0,0.94), rgba(55,0,0,0.88));
    border: 1px solid rgba(255,26,26,0.48); border-radius: 16px; padding: 2.5rem; text-align: center;
    box-shadow: 0 0 45px rgba(255,26,26,0.14);
    animation: result-pop 0.4s ease both;
}}
...
.stButton > button {{
    ...
    box-shadow: 0 0 18px rgba(0,80,255,0.3) !important;
}}
.stButton > button:hover {{
    background: linear-gradient(135deg, rgba(0,120,255,0.98), rgba(255,0,220,0.98)) !important;
    box-shadow: 0 0 32px rgba(255,0,200,0.5) !important;
...
.stTextInput > div > div > input {{
    ...
    font-size: 0.8rem !important; transition: border-color 0.2s, box-shadow 0.2s !important;
}}
...
/* ── TABS as STICKY NAVBAR ── */
div[data-baseweb="tab-list"] {{
    background: rgba(1,2,4,0.96) !important;
    border-bottom: 1px solid rgba(0,245,255,0.13) !important;
    border-radius: 0 !important; gap: 0 !important; padding: 0 1rem !important;
    position: sticky; top: 0; z-index: 999;
    box-shadow: 0 4px 24px rgba(0,0,0,0.65);
}}
div[data-baseweb="tab"] {{
    font-family: 'Orbitron', sans-serif !important; font-size: 0.67rem !important;
    letter-spacing: 2.5px !important; color: var(--text-muted) !important;
    border-radius: 0 !important; padding: 1rem 1.5rem !important;
    border-bottom: 2px solid transparent !important;
    transition: color 0.2s, background 0.2s !important; text-transform: uppercase !important;
}}
...
div[aria-selected="true"] {{
    ...
    border-bottom: 2px solid var(--neon-cyan) !important; box-shadow: none !important;
}}
...
div[data-testid="stFileUploader"] {{ background:rgba(0,16,32,0.55) !important; border:1px dashed rgba(0,245,255,0.28) !important; border-radius:10px !important; transition:border-color 0.2s !important; }}
...
.watermark {{
    ...
    color:rgba(0,245,255,0.68); text-align:right; line-height:1.65; pointer-events:none;
    border-left:2px solid rgba(0,245,255,0.52); padding-left:9px; text-transform:uppercase;
...
def plotly_gauge(confidence, label):
    ...
    fig   = go.Figure(go.Indicator(
        ...
        gauge={
            ...
            "steps":[
                {"range":[0,40],   "color":"rgba(57,255,20,0.05)"},
    ...
    fig.update_layout(paper_bgcolor=_PBG, plot_bgcolor=_PBG,
                      margin=dict(t=28,b=8,l=8,r=8), height=220, font=_PFNT)
    return fig
...
def plotly_frame_line(frame_probs, threshold):
    ...
    fig.add_trace(go.Scatter(
        x=xs, y=frame_probs, mode="lines+markers",
...
def plotly_donut(confidence, label):
    ...
    fig    = go.Figure(go.Pie(
        values=[real_p, 100-real_p], labels=["Real","Fake"], hole=0.68,
...
def show_result(label, confidence, frame_probs, threshold):
    if label == "Real":
        ...
        <div class="result-real">
            ...
            <div style="margin-top:0.8rem;font-family:'Share Tech Mono',monospace;font-size:0.62rem;
                ...
                NO SYNTHETIC SIGNATURES DETECTED</div>
        </div>""", unsafe_allow_html=True)
    else:
        st.markdown(f"""
        <div class="result-fake">
            ...
            <div style="margin-top:0.8rem;font-family:'Share Tech Mono',monospace;font-size:0.62rem;
                        letter-spacing:3px;color:rgba(255,26,26,0.5);">
                SYNTHETIC MANIPULATION DETECTED</div>
        </div>""", unsafe_allow_html=True)
    ...
    <div class="stat-grid">
        <div class="stat-box">
            <span class="s-val">{fake_frames}/{len(frame_probs)}</span>
            <span class="s-lbl">Flagged Frames</span>
        </div>
        <div class="stat-box">
            <span class="s-val">{float(np.mean(frame_probs)):.2f}</span>
            <span class="s-lbl">Avg Fake Score</span>
        </div>
        <div class="stat-box">
            <span class="s-val">{float(np.std(frame_probs)):.2f}</span>
            <span class="s-lbl">Score Volatility</span>
        </div>
    </div>""", unsafe_allow_html=True)
...
def hero_illustration():
    ...
    <svg viewBox="0 0 520 140" xmlns="http://www.w3.org/2000/svg"
         style="width:100%;max-width:500px;opacity:0.44;">
      <defs>
      ...
      <ellipse cx="260" cy="70" rx="46" ry="28" stroke="#ff1a1a" stroke-width="1.1"
               fill="none" opacity="0.48" filter="url(#gF)"/>
      <ellipse cx="260" cy="70" rx="26" ry="16" stroke="#00f5ff" stroke-width="0.8"
               fill="none" opacity="0.55" filter="url(#gF)"/>
    ...
    with gcol1:
        ...
        for num, title, body in [
            ...
            ("05","Interpret the Graphs",
             "Gauge = overall confidence. Line chart = per-frame fake probability. "
             "Bar chart = per-frame risk. Histogram = score distribution."),
            ("06","Take Action",
             "If flagged as fake, verify the source, report to the platform, and file a "
             "cyber complaint using the Awareness & Help tab."),
        ]:
            st.markdown(f"""
    ...
    with acol2:
        ...
        for title, body in [
            ("Verify Before You Share",
             ...
             "before sharing anything that seems extraordinary or emotionally charged."),
            ("Look for Visual Artefacts",
             "Unnatural blinking, mismatched lighting, blurry hairlines, and irregular skin "
             "texture are common tells. Slow to 0.25× to reveal frame glitches."),
    ...
    with pcol1:
        ...
        for num, title, body in [
            ...
            ("02","Report to Platform",
             "Use the platform's Report feature (YouTube, Meta, TikTok, X). "
             "Select 'Fake or deceptive content' / 'Deepfake' / 'Synthetic media'."),
            ...
            ("04","Visit Nearest Cyber Police Station",
             "File an FIR at your local cyber crime cell with printed evidence "
             "and your online complaint registration number."),
            ("05","Follow Up",
...
<div class="footer-bar">
    ...
    <span>DFD Dataset · MobileNetV2 · Frame-level Analysis</span>
</div>""", unsafe_allow_html=True)